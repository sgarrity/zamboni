var plotSelection = {
    ...
    loadSummary: function() {
        $.getJSON(statsURL + 'json/' + addonID + '/summary', function(data) {
            ...
            if (Plots.dataTable) {
                Plots.dataTable.config['headerTranslations'] = plotSelection.summary.prettyNames;
            }
        });
    ...
    addPlotSelector: function() {
        ...
        menu.addItem({'value': 'downloadsource', 'name': localized['statistics_js_plotselection_selector_downloadsource'], 'indented': true});
        ...
        menu.addItem({'value': 'locale', 'name': localized['statistics_js_plotselection_selector_locale'], 'indented': true});
            ...
            $.each(values, function(item, count) {
                if (typeof count != 'object') {
                    ...
                    typeItem.submenu.addItem({
                            'value': item,
                            'name': (item in plotSelection.summary.prettyNames ? plotSelection.summary.prettyNames[item] : item),
    ...
    addDefinedDropdown: function(selectedIndex) {
        ...
        for (var j = 2; j < Plots.availableFields.length; j++) {
            if (type == 'application') {
                ...
                item = menu.addItem({
                        'value': Plots.availableFields[j],
                        'name': (Plots.availableFields[j] in plotSelection.summary.prettyNames ? plotSelection.summary.prettyNames[Plots.availableFields[j]] : Plots.availableFields[j]),
                        'prefix': (plotSelection.summary.shortNames[type] ? plotSelection.summary.shortNames[type] + ':' : '')
                    });
    ...
    definedChanged: function(dropdown) {
        ...
        // Recolor download source overview keys
        if (dropdown.config.type == 'downloadsource') {
            plotSelection.recolorDownloadsourceOverview();
        }
    },
    ...
    recolorDownloadsourceOverview: function() {
        // reset all color keys to white
        $('#stats_downloadsource_overview .colorbox').css('background-color', '#fff');

        for (var plot in this.dropdowns) {
            if (this.dropdowns[plot] == null) {
                continue; // sadly old dropdowns stick around
            }
            if (this.dropdowns[plot].config['type'] != 'downloadsource') {
                continue; // not a downloadsource dropdown
            }
            if (! ('value' in this.dropdowns[plot].selectedItem)) {
                continue; // dropdown is turned off
            }

            var source = this.dropdowns[plot].selectedItem['value'];
            var color = this.dropdowns[plot].config['color'];
            $('#stats_downloadsource_overview .colorbox.'+source+'-source').css('background-color', color);
        }
    }
};
