var Plots = {
    ...
    summary: {
        ...
        updateOptionsVisibility: function() {
            ...
            if (plotType == 'summary') {
                ...
                $('#stats-table-container').children().hide();
                $('#stats_overview').show();
            }
            ...
            else if (plotType == 'downloadsource') {
                $('#weeks-legend').hide();
                $('#summary-legend').hide();
                $('#summary-options').hide();
                $('#group-by-selector').show();
                $('#stats-table-container').children().hide();
                if ($('#stats_downloadsource_overview').children().length) {
                    $('#stats_downloadsource_overview').show();
                } else {
                    this.fetchDownloadsourceOverview();
                }
            }
            else {
                $('#weeks-legend').hide();
        ...
        fetchDownloadsourceOverview: function() {
            $.get(statsURL + 'overview/' + addonID + '/downloadsource', function(data, status){
                if (status == 'success') {
                    $('#stats_downloadsource_overview').html(data);
                    $('a#stats-csv-downloadsource').attr('href', Plots.currentCSV);
                    $('#stats_downloadsource_overview a.explain').click(function() {
                        $(this).next('.explanation').slideToggle('fast');
                        return false;
                    });
                    $('a.show-collections').click(function() {
                        $('a.show-collections').fadeOut('fast', function(){
                            $('#stats_downloadsource_overview tr.collection-row:hidden, a.hide-collections').fadeIn('fast');
                        });
                        return false;
                    });
                    $('a.hide-collections').click(function() {
                        $('#stats_downloadsource_overview tr.collection-row:visible, a.hide-collections').fadeOut('fast', function(){
                            $('a.show-collections').fadeIn('fast');
                        });
                        return false;
                    });
                    // just in case this request finished before the csv request
    ...
    defined: {
        ...
        plot: function() {
            ...
            Plots.newTimeplot();
            var plotType = plotSelection.dropdowns['plot-selector'].selectedItem.value;
            if (Plots.dataTable && plotType != 'downloadsource') {
