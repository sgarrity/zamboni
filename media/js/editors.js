...
var editors_queue = {
    init: function() {
        // setup hide/show label in basic search input
        var q = $('#FilterAddonOrAuthor');
        var l = $('#queue-search-basic label[for=FilterAddonOrAuthor]');
        q.focus(function() {
            l.hide();
        });
        q.blur(function() {
        ...
        // setup hide/show advanced search fields
        $('#queue-search-advanced-link a').click(function() {
            if ($(this).hasClass('asclosed')) {
                $(this).removeClass('asclosed').addClass('asopen');
            } else {
                $(this).removeClass('asopen').addClass('asclosed');
            }
            $('#queue-search-advanced').toggle('fast');
            return false; 
        });
...
// insert into a textarea at the cursor location (in front of any selection)
